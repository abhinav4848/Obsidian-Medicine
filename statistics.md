...
- Pearson's product-moment coefficient is a parametric test assessing correlation. The type of significance test used depends on whether the data is parametric (something which can be measured, usually normally distributed) or non-parametric.
...
Does not follow a normal distribution. Might be skewed, have outliers, or be based on ranks/ categories. e.g. Pain score (0-10), Likert scales, time to recovery. 
- Mann–Whitney U test = comparing ranked scores. This is for non-parametric data and compares ordinal, interval, or ratio scales of unpaired data. 
